...
def count_ngrams(snt, max_n):
    ...
    for i in range(len(snt)):
        for k in range(i + 1, min(i + max_n + 1, len(snt) + 1)):
            key = tuple(snt[i:k])
...
class BleuScore:
    def __init__(self, hyp, ref, max_n=4, bootstrap=1000):
        ...
        self.upper = None
        self.median = None
        self.actual = self.score([i for i in range(len(hyp.snt))])
        if bootstrap:
    ...
    def score(self, sample):
        hits = [0 for i in range(self.max_n)]
        self.hyplen = 0
        self.reflen = 0
...
class Document:
    def __init__(self, fname=None, lines=None):
        ...
        if fname:
            ...
            self.snt = [line.strip().split() for line in lines]
            self.ngrams = [count_ngrams(snt, 4) for snt in self.snt]
        else:
            self.snt = None
            self.ngrams = None
...
def bleu(ref,cand, individual, bootstrap=1000, files = True, alpha=.05):
    if files:
        R = [Document(fname =fname) for fname in ref]
        C = [Document(fname = fname) for fname in cand]
    else:
        R = [Document(lines=ref)]
        C = [Document(lines=cand)]
    bleu = 0
    ...
    for c in C:
        ...
        Rx.update(c, R)
        bleu = BleuScore(c, Rx, bootstrap=bootstrap)
        print("%5.2f %s [%5.2f-%5.2f; %5.2f] %s" % (
            100 * bleu.actual,
            os.path.basename(Rx.fname),
            100 * bleu.bootstrap[int((alpha / 2) * bootstrap)],
            100 * bleu.bootstrap[int((1 - (alpha / 2)) * bootstrap)],
            100 * bleu.bootstrap[int(.5 * bootstrap)],
            c.fname))  # os.path.basename(c.fname))

        if individual:
            for r in R:
                bleu = BleuScore(c, r, bootstrap=bootstrap)
                print("  %5.2f %s" % (
                    100 * bleu.actual, os.path.basename(r.fname)))
                # print bleu.prec, bleu.hyplen, bleu.reflen, bleu.BP
    return 100 * bleu.actual
        # print [
...
if __name__ == "__main__":
    ...
    bleu(args.ref,args.cand, args.individual, args.bootstrap, args.alpha)
